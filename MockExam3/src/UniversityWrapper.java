...
class UniversityWrapper {
	...
	public static class University {
		...
		public boolean takeSameCourse(Student s1, Student s2){
			for(Course c: courses){
				if((c.findRollBookEntry(s1.getID())!=null) && (c.findRollBookEntry(s2.getID()) !=null)){
					return true;
				}
	...
	public static class StaffMember implements Person{
		...
		public String getType(){return "StaffMember";}
	...
	public static class Student implements Person{
		...
		public void drop(Course c) {//someStudent.drop(someCourse)
			int arrayLocation = c.rollBookPos(getID());
			c.rollBook[arrayLocation]=c.rollBook[c.rollBook.length-1];
			c.numStudents--;
		}
	}
	...
	public static class Course {
		...
		public int rollBookPos(int studentID){
			for (int i=0; i< numStudents; i++) {
				if (rollBook[i].getStudent().getID() == studentID) { return i; }
			}
			return -1;
		}
	...
	public static class GradCourse extends Course{
		...
		public void enroll(Student s){
			if(s.isGraduate()){
				if (this.getNumStudents() < getRollBook().length) {
					getRollBook()[super.numStudents++] = new RollBookEntry(s);
					s.setCredits(s.getCredits()+getCredits());
				}
				else {
					System.out.println("Fatal internal error: Attemp to register student in full course"); 
